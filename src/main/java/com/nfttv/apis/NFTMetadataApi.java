...
class NFTMetadataApi {
    ...
    public ResponseEntity<NftApiResource> getNFTs(@RequestParam(required = false) Integer page,
        ...
        final Set<NftMetadataApiResource> metadata = filterNft(pageable).stream().map(this::buildMetadataResource).collect(Collectors.toSet());
    ...
    public ResponseEntity<Page<NftMetadataApiResource>> filter(@RequestParam(required = false) Integer page,
        ...
        final Page<Nft> nfts = filterNft(pageable);
    ...
    Page<Nft> filterNft(final Pageable pageable) {
